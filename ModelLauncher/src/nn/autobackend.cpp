...
namespace {
	void loadLabelsFromDataset(fs::path datasetPath, std::unordered_map<int, std::string>& labels)
	{
		fs::path path = datasetPath;
		
		std::ifstream classesFile(datasetPath / "predefined_classes.txt");
		if (!classesFile.is_open())
			throw ModelLauncherException("Can't open predefined_classes.txt");

		std::string line;
		size_t classType;
		std::string classLabel;
		auto reserved = std::move(labels);
		while (getline(classesFile, line)) {
			...
			if (labels.find(classType) != labels.end()) {
				labels = reserved;
				throw ModelLauncherException("There are 2 classes with 1 label - "
					+ std::to_string(classType));
			}

			labels[classType] = classLabel;
		}
	}
}

AutoBackendOnnx::AutoBackendOnnx(const char* modelPath, const char* datasetPath, const char* logid, const char* provider)
	...
	if (names_item != base_metadata.end()) {
		...
		loadLabelsFromDataset(datasetPath, names_);
		// Added
		/*names_ = {
			{ 0, "Lymphocyte" },
			{ 1, "Metamyelocyte" },
	...
	int& class_names_num, float& conf_threshold, float& iou_threshold)
{
	...
	for (int r = 0; r < rows; ++r)
	{
		...
		if (max_conf > conf_threshold)
		{
			...
			float out_w = pdata[2];
			float out_h = pdata[3];
			float out_left = MAX((pdata[0] - 0.5f * out_w + 0.5f), 0);
			float out_top = MAX((pdata[1] - 0.5f * out_h + 0.5f), 0);
